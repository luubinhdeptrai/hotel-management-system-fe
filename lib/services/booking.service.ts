...
export const bookingService = {
  ...
  /**
   * Confirm a pending booking (MOCK)
   * PATCH /employee/bookings/{id}/confirm (Endpoint does not exist)
   *
   * @deprecated Backend does not support specific "confirm" action.
   * Booking status should be managed via updateBooking or check-in.
   * This mock is kept for backward compatibility with older hooks.
   */
  async confirmBooking(bookingId: string): Promise<ConfirmBookingResponse> {
    console.warn("confirmBooking is a MOCK function. No backend call made.");
    return {
      id: bookingId,
      bookingCode: "",
      status: "CONFIRMED",
  ...
  async getAvailableRooms(
    ...
    try {
      ...
      if (response && typeof response === "object" && "data" in response) {
        ...
        if (
          firstUnwrap &&
          typeof firstUnwrap === "object" &&
          "data" in firstUnwrap
        ) {
          groupedArray = firstUnwrap.data;
          console.log(
            "After second unwrap (response.data.data):",
            groupedArray
          );
  ...
  flattenGroupedRooms(data: any[]): AvailableRoom[] {
    ...
    data.forEach((group, idx) => {
      ...
      if (group && group.rooms && Array.isArray(group.rooms)) {
        console.log(
          `  - Found grouped format with ${group.rooms.length} rooms`
        );
        ...
        console.warn(
          `  - Skipped unrecognized group format at index ${idx}:`,
          group
        );
      }
    });
  ...
  // ============================================================================
  // BOOKING SERVICES (DEPRECATED - Use service-unified.service.ts)
  // ============================================================================

  /**
   * Add service to a booking
   * @deprecated Use serviceUsageAPI.createServiceUsage() instead
   */
  async addService(
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _bookingId: string,
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _data: AddServiceRequest
  ): Promise<BookingService> {
    throw new Error("Deprecated: Use serviceUsageAPI.createServiceUsage()");
  },

  /**
   * Get all services for a booking
   * @deprecated Use serviceUsageAPI.getServiceUsages() instead
   */
  async getBookingServices(
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _bookingId: string
  ): Promise<BookingServicesResponse> {
    throw new Error("Deprecated: Use serviceUsageAPI.getServiceUsages()");
  },

  /**
   * Remove service from a booking
   * @deprecated Use serviceUsageAPI.deleteServiceUsage() instead
   */
  async removeService(
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _bookingId: string,
    ...
    throw new Error("Deprecated: Use serviceUsageAPI.deleteServiceUsage()");
  },
  ...
  async getCancellationPreview(
    ...
    throw new Error(
      "Cancellation preview is not supported by Backend. Backend does not have cancellation policy or refund calculation."
    );
  },
  ...
  async changeRoom(
    ...
    try {
      const response = await api.post<
        ApiResponse<{ bookingRoom: BookingRoom; priceAdjustment: any }>
      >(`/employee/bookings/rooms/${bookingRoomId}/change-room`, data, {
        requiresAuth: true,
      });
      const unwrappedData =
        response && typeof response === "object" && "data" in response
