...
export interface GetPricingRulesParams {
  page?: number;
  limit?: number;
 ...
 *
 * @param params - Query parameters (page, limit, includeInactive)
 * @returns Paginated pricing rules or array
 */
export async function getPricingRules(
  params: GetPricingRulesParams = {}
): Promise<PaginatedResponse<PricingRule> | PricingRule[]> {
  const url = new URL(`${API_BASE_URL}${API_PREFIX}`);

  if (params.page) url.searchParams.set("page", String(params.page));
  if (params.limit) url.searchParams.set("limit", String(params.limit));
  if (params.includeInactive !== undefined)
    url.searchParams.set("includeInactive", String(params.includeInactive));
  ...
  // Handle potential paginated response structure consistent with other services
  if (
    data &&
    typeof data === "object" &&
    "data" in data &&
    ...
    return data; // It's likely a PaginatedResponse structure: { data: [], total, page, limit }
    ...
    // Handles unwarpped { data: { data: [], ... } } if backend wraps it twice
 ...
 * Backend expects "newPosition" (index)
 *
...
export async function getCalendarEvents(): Promise<CalendarEvent[]> {
  const response = await fetch(`${API_BASE_URL}/employee/calendar-events`, {
    method: "GET",
    headers: getAuthHeaders(),
  });
